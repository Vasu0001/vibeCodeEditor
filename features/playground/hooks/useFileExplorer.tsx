...
const getFilePath = (parentPath: string, file: TemplateFile) => {
  if (file.path) return normalizePath(file.path);
  const filename = getFileName(file);
  return normalizePath(parentPath ? `${parentPath}/${filename}` : filename);
};
...
export const useFileExplorer = create<FileExplorerState>((set, get) => ({
  ...
  setPlaygroundId(id) {
    const current = get().playgroundId;
    if (current === id) return;

    // Prevent state leakage when switching playgrounds
    set({
      playgroundId: id,
      templateData: null,
      openFiles: [],
  ...
  handleAddFile: async (newFile, parentPath, writeFileSync, instance, saveTemplateData) => {
    ...
    try {
      ...
      get().openFile({ ...newFile, path: filePath });
    } catch (error) {
  ...
  handleDeleteFile: async (file, parentPath, instance, saveTemplateData) => {
    ...
    try {
      ...
      // Sync delete with web container
      try {
        if (instance?.fs) {
          await instance.fs.rm(filePath, { force: true });
        }
      } catch {
        // best-effort (container may not be ready yet)
      }
    ...
    try {
      ...
      const closeFilesInFolder = (folder: TemplateFolder, currentPath: string = "") => {
        folder.items.forEach((item) => {
          if ("filename" in item) {
            const fileId = getFilePath(currentPath, item);
            get().closeFile(fileId);
      ...
      // Sync delete with web container (recursive)
      try {
        const folderPath = parentPath
  ...
  handleRenameFile: async (
    ...
    const newFile = { ...file, filename: newFilename, fileExtension: newExtension, path: undefined };
    ...
    try {
      ...
      if (fileIndex !== -1) {
        ...
        set({
          ...
          activeFileId: activeFileId === oldPath ? newPath : activeFileId,
        });

        // Sync rename with web container
        try {
          if (instance?.fs) {
  ...
  handleRenameFolder: async (folder, newFolderName, parentPath, instance, saveTemplateData) => {
    ...
    try {
      const oldPath = normalizePath(parentPath ? `${parentPath}/${folder.folderName}` : folder.folderName);
      const newPath = normalizePath(parentPath ? `${parentPath}/${newFolderName}` : newFolderName);
      const updatedTemplateData = JSON.parse(
        JSON.stringify(templateData)
      ) as TemplateFolder;
      const pathParts = parentPath.split("/");
      ...
      if (folderIndex !== -1) {
        ...
        const updatedOpenFiles = openFiles.map((file) => {
          if (file.id === oldPath || file.id.startsWith(`${oldPath}/`)) {
            const nextId = file.id.replace(oldPath, newPath);
            return {
              ...file,
              id: nextId,
              path: nextId,
        ...
        set({
          ...
          openFiles: updatedOpenFiles,
          activeFileId:
            activeFileId && (activeFileId === oldPath || activeFileId.startsWith(`${oldPath}/`))
              ? activeFileId.replace(oldPath, newPath)
              : activeFileId,
        });
