...
function TemplateNode({
  ...
  if (!isFolder) {
    ...
    const isSelected =
      selectedFile &&
      ((selectedFile.path && selectedFile.path === filePath) ||
        (!selectedFile.path &&
    ...
    const confirmDelete = () => {
      onDeleteFile?.(fileWithPath, path)
      setIsDeleteDialogOpen(false)
    }

    const handleRenameSubmit = (newFilename: string, newExtension: string) => {
      onRenameFile?.(fileWithPath, newFilename, newExtension, path)
    ...
    return (
      <SidebarMenuItem>
        <div className="flex items-center group">
          <SidebarMenuButton isActive={isSelected} onClick={() => onFileSelect?.(fileWithPath)} className="flex-1">
